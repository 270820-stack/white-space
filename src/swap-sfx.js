...
function digitalTick(audio, dest, t, freq, len) {
  const osc = audio.createOscillator();
  const gain = audio.createGain();
  osc.type = "square";
  osc.frequency.setValueAtTime(freq, t);
...
function codeStream(audio, dest, t, dur) {
  let u = t;
  while (u < t + dur) {
    const burst = 3 + ((Math.random() * 5) | 0);
    for (let i = 0; i < burst; i++) {
      const freq = 420 + Math.random() * 3200;
      digitalTick(audio, dest, u + i * 0.016, freq, 0.016 + Math.random() * 0.028);
    }
    u += 0.07 + Math.random() * 0.09;
  }
}

function lockChirp(audio, dest, t) {
  const steps = [740, 980, 1310, 1760];
  steps.forEach((freq, i) => {
    digitalTick(audio, dest, t + i * 0.055, freq, 0.05);
  });
}

export function playSelectLock() {
  const audio = getAudioContext();
  if (audio.state === "suspended") audio.resume().catch(() => {});
  const t = audio.currentTime;
  const bus = getAnimBus();
  lockChirp(audio, bus, t);
  for (let i = 0; i < 8; i++) {
    digitalTick(audio, bus, t + 0.22 + i * 0.04, 900 + Math.random() * 1600, 0.02);
  }
}
